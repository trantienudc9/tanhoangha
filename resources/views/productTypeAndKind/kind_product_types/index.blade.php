<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-6 py-8 bg-white from-blue-100 via-purple-100 to-pink-100 rounded-xl shadow-lg">
        ...
        @endif
        @php
            $edit = isset($getFindProduct) ? route('kind-product-types.update') : route('kind-product-types.store');
        ...
        <form action="{{ $edit }}" method="POST" class="space-y-6">
            ...
            <label for="select" class="block text-gray-800 text-xl font-semibold mb-4">Sản phẩm</label>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                <!-- Select -->
                <div class="flex items-center gap-4">
                    <label for="select" class="text-gray-800 font-medium">Loại</label>
                    <select id="select"
                ...
                <div class="flex items-center gap-4">
                    <label for="name" class="text-gray-800 font-medium">Tên sản phẩm</label>
                    <input type="text" id="name" name="name" value="{{ isset($getFindProduct->name) ? $getFindProduct->name : '' }}" class="block w-full md:w-2/3 p-3 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-400 focus:border-transparent text-gray-700" placeholder="Tên sản phẩm"/>
                </div>
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
                <thead>
                    <tr class="bg-teal-500 text-white">
                        ...
                        <th class="py-3 px-4 !text-center">Tên sản phẩm</th>
                ...
                <tbody>
                    @foreach ($productTypes as $key => $productType)
                        <tr class="{{ $background == $productType->id ? '!bg-teal-300' : '' }} even:bg-gray-200 odd:bg-gray-50 hover:bg-teal-200 border-t border-gray-200">
                            ...
                            <td class="py-3 px-4 text-center">{{ $productType->productType->name }}</td>
                            <td class="py-3 px-4 text-center">{{ $productType->name }}</td>
                            <td class="py-3 px-4 text-center">
                                <a href="{{ route('kind-product-types.index', ['id' => $productType->id]) }}">
                                    <i class="fa-solid fa-pen-to-square text-yellow-400 text-xl mr-2"></i>
                                </a>
                                <button type="button" class="delete_productType" onclick="deleteForm('{{ route('kind-product-types.destroy') }}',{{ $productType->id }})">
